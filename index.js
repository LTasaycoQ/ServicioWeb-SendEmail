...
app.post('/contact', async (req, res) => {
  ...
  try {
    await transporterGeneral.sendMail({
      from: `"Peru Luxury" <${USER_1}>`,
      to: USER_1,
      subject: 'Request a Personalized Itinerary',
      html: `
        <div>
          <h2>Solicitud de Itinerario Personalizado</h2>
          <p style="display:flex; gap:4px;"><span style="font-weight:bold;">Nombre</span> ${nombre}</p>
          <p style="display:flex; gap:4px;"><span style="font-weight:bold;">Correo</span> ${email}</p>
        </div>
